						...
						.forEach((x:any,i)=>{
							...
							if(["target","part"].includes(zChildren[x].extras?.appLatch?.display?.type)){
								...
								if(["target"].includes(zChildren[x].extras?.appLatch?.display.type)){
									...
									if(!zChildren[x].extras?.appLatch.zChildren){
										throw("zChildren needs to be defined and to be an array according to the zProtoChildren Interface defined in customExports")
									}
