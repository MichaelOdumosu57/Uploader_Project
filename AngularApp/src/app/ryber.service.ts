...
export class RyberService {
    ...
    appCO0: Partial<componentObject> = {
        metadata: {
            ...
            particlesjs:{
                current:new ReplaySubject(1),
                ngOnDestroy:new BehaviorSubject({
                    hook:"psuedoFinished"
                })
            },
            facebookLogin:{
    ...
    unauthenticated= (err)=>{
        ...
        if(err.statusText ==="Unauthorized"){
            this.appCO0.metadata.navigation.full.navigated = "true"
            this.appCurrentNav = "/login"
            alert("Please Log In Again")
            this.ref.detectChanges()
        }
        throw('e')
    }
    authAS$ =  (devObj:authAE)=>{
        ...
        return this.http.post(
            env.backend.url,
            {
                env:"authConversion",
                ...devObj,
                user:this.appCO0.metadata.facebookLogin.loginName,
                access_token:this.appCO0.metadata.facebookLogin.accessToken
            }
        )
        .pipe(retry(2),catchError(this.unauthenticated))
    }
    //
